...
PROMETHEUS_URL = os.environ.get('PROMETHEUS_URL', 'https://prometheus.onrender.com')
...
class PrometheusManager:
    ...
    def get_available_metrics(self):
        ...
        response = requests.get(f"{self.url}/api/v1/label/__name__/values")
        return response.json()

# Initialize managers
grafana = GrafanaManager(GRAFANA_URL, GRAFANA_USER, GRAFANA_PASSWORD)
prometheus = PrometheusManager(PROMETHEUS_URL)
...
def analyze_with_openai(user_message, context=""):
    """Analyze user request with OpenAI"""
    system_prompt = f"""
...
def chat():
    try:
        ...
        # Get context from Grafana and Prometheus
        context = ""
        try:
        ...
        try:
            metrics = prometheus.get_available_metrics()
            if 'data' in metrics:
                context += f"Available metrics: {len(metrics['data'])} metrics found. "
        except:
            context += "Prometheus connection issue. "
...
def health():
    return jsonify({
        'status': 'healthy',
        'service': 'OpenAI AI Observability Platform',
        'timestamp': datetime.now().isoformat(),
        'features': [
