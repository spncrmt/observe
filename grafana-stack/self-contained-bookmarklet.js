// Self-Contained AI Assistant Bookmarklet
...
javascript:(function(){
    ...
    // AI Response Logic (Built-in)
    function getAIResponse(userMessage) {
        const message = userMessage.toLowerCase();
        
        if (message.includes('create') || message.includes('build') || message.includes('make') || message.includes('dashboard')) {
            return `I'll create a CPU usage dashboard for you! Here's what I'm building:
...
Would you like me to create this dashboard now?`;
        ...
        if (message.includes('analyze') || message.includes('analysis') || message.includes('performance') || message.includes('health')) {
...
Would you like me to create these alerts for you?`;
        ...
        if (message.includes('query') || message.includes('promql') || message.includes('metric')) {
            return `Here are some useful PromQL queries for your monitoring:
...
- "Build a comprehensive monitoring dashboard"`;
    ...
    function addMessage(content, isUser = false) {
        ...
        let formattedContent = content
            ...
            .replace(/```([\s\S]*?)```/g, '<pre style="background: #1a1a1a; padding: 0.5rem; border-radius: 0.25rem; overflow-x: auto; margin: 0.5rem 0;"><code>$1</code></pre>')
            .replace(/\n/g, '<br>');
