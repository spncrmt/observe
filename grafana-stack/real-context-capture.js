// Real Grafana Context Capture - Based on Actual Inspection
(function() {
  const AI_SERVICE_URL = 'http://localhost:5001';
  
  // Enhanced context capture based on actual Grafana structure
  function captureRealGrafanaContext() {
    ...
    console.log('🔍 Capturing real Grafana context...');
    
    // Method 1: Extract from grafanaBootData (most reliable)
    if (window.grafanaBootData) {
      console.log('✅ Found grafanaBootData:', window.grafanaBootData);
      ...
      if (window.grafanaBootData.settings?.dashboard) {
        ...
        console.log('✅ Found dashboard from settings:', context.dashboard_title);
      }
    }
    ...
    if (!context.dashboard_id) {
      ...
      if (urlMatch) {
        ...
        console.log('✅ Found dashboard ID from URL:', context.dashboard_id);
      }
    }
    ...
    if (!context.dashboard_title) {
      ...
      console.log('✅ Found dashboard title from page title:', context.dashboard_title);
    }
    ...
    if (panelElements.length > 0) {
      console.log('✅ Found panel elements:', panelElements.length);
    ...
    if (queryElements.length > 0) {
      console.log('✅ Found query elements:', queryElements.length);
    ...
    if (timeElements.length > 0) {
      ...
      console.log('✅ Found time range:', context.time_range);
    }
    ...
    if (window.grafanaBootData?.navTree) {
      ...
      console.log('✅ Found data sources:', context.available_data_sources);
    }
    
    console.log('📊 Final real context:', context);
    return context;
  }
  
  // Function to update AI service with real context
  function updateAIWithRealContext(context) {
    ...
    .then(data => {
      if (data.success) {
        console.log('✅ AI context updated with real data:', data.message);
      } else {
  ...
  // Function to create enhanced AI chat with real context
  function createEnhancedAIChat() {
    const chatDiv = document.createElement('div');
    chatDiv.id = 'enhanced-ai-chat';
    ...
    chatDiv.innerHTML = `
      <div style='background: #3d3d3d; padding: 15px; border-radius: 8px 8px 0 0; border-bottom: 1px solid #4d4d4d;'>
        <div style='display: flex; justify-content: space-between; align-items: center;'>
          ...
          <button id='close-ai-chat' style='background: none; border: none; color: #ff6b6b; font-size: 20px; cursor: pointer;'>×</button>
        </div>
      ...
      <div style='padding: 15px; background: #3d3d3d; border-radius: 0 0 8px 8px;'>
        <div style='display: flex; gap: 10px; margin-bottom: 10px;'>
          <input id='ai-input' type='text' placeholder='Ask about your dashboard, create panels, analyze metrics...' 
        ...
        <div style='display: flex; gap: 5px; flex-wrap: wrap;'>
          <button id='capture-context' style='padding: 6px 12px; background: #4d4d4d; color: white; border: none; border-radius: 3px; cursor: pointer; font-size: 11px;'>Capture Context</button>
          ...
          <button id='test-ai' style='padding: 6px 12px; background: #4d4d4d; color: white; border: none; border-radius: 3px; cursor: pointer; font-size: 11px;'>Test AI</button>
        </div>
      </div>
    `;
  ...
  function initEnhancedChat() {
    ...
    const captureBtn = document.getElementById('capture-context');
    ...
    function sendMessage() {
      ...
      loadingDiv.textContent = '🤔 Processing with real dashboard context...';
      messagesDiv.appendChild(loadingDiv);
      
      // Capture real context before sending
      currentContext = captureRealGrafanaContext();
      updateAIWithRealContext(currentContext);
      ...
      .catch(error => {
        messagesDiv.removeChild(loadingDiv);
        addMessage('❌ Error connecting to AI agent. Check if the service is running on port 5001.', false, null, false);
      });
    }
    ...
    fetch(`${AI_SERVICE_URL}/health`)
      ...
      .then(data => {
        ...
        addMessage(`Hello! I'm your AI observability agent with **real dashboard context awareness**. 
...
• Data Sources: ${currentContext.available_data_sources.length}

I can help you:
• Create panels and dashboards
...
What would you like to do?`, false);
    ...
    captureBtn.onclick = () => {
      ...
      updateAIWithRealContext(currentContext);
      updateContextDisplay();
      addMessage('✅ Real dashboard context captured and sent to AI agent.', false, 'context_update', true);
    };
    
    showContextBtn.onclick = () => {
      if (currentContext) {
        const contextInfo = `**Current Real Context:**
• Dashboard: ${currentContext.dashboard_title || 'Unknown'}
...
• Grafana Version: ${currentContext.grafana_version || 'Unknown'}`;
        ...
        addMessage('❌ No context available. Click "Capture Context" first.', false, null, false);
      }
    };
    
    testBtn.onclick = () => {
      addMessage('🧪 Testing AI with real context...', false, 'test', true);
      sendMessage();
    };
  ...
  aiButton.onclick = function() {
    const existingChat = document.getElementById('enhanced-ai-chat');
    if (existingChat) {
      existingChat.remove();
    } else {
      createEnhancedAIChat();
    }
  };
  ...
  setTimeout(() => {
    ...
    updateAIWithRealContext(initialContext);
  }, 2000);
  
  console.log('🎯 Real Grafana Context Capture loaded! Based on actual inspection data.');
})(); 