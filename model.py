...
def _calculate_rolling_statistics(
    ...
    df[f'{column}_rolling_mean'] = df[f'{column}_rolling_mean'].fillna(method='ffill')
    df[f'{column}_rolling_std'] = df[f'{column}_rolling_std'].fillna(method='ffill')
...
def detect_anomalies_custom(
    ...
    try:
        ...
        result_df['correlated_errors'] = result_df['correlated_errors'].fillna([])
